...
from datetime import datetime, timedelta  # ✅ Added timedelta for response time calculation
...
def chatbot_response():
    ...
    try:
        ...
        print(f"[{timestamp}] Response time: {response_time.total_seconds():.2f} seconds")

        return jsonify({"answer": response, "response_time": f"{response_time.total_seconds():.2f} sec"})
    except Exception as e:
        return jsonify({"answer": f"Error: {str(e)}"}), 500

#  Added health check endpoint with request count info
